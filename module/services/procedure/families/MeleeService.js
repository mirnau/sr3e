...
export default class MeleeService {
  ...
  static planStrike({ attacker, defender, weapon, situational = {} }) {
    ...
    // Reach: advantage lowers TN for the longer-reach side and raises for the other
    if (reachDiff !== 0) {
    ...
    // Called shot (+4 TN). If you house-rule extra staging, tweak levelDelta here.
    ...
    // Generic and position TN adds (computed by caller)
    if (this.#num(situational?.tnAdd) !== null) attackerTNAdd += this.#num(situational.tnAdd);
    if (this.#num(situational?.positionTNAdd) !== null) attackerTNAdd += this.#num(situational.positionTNAdd);

    // Collect melee directives (adept powers, materials, qualities, etc.)
    ...
    // Assemble DamagePacket; default armor.use=impact (overridable by directives)
    const packet = this.#buildMeleePacket({ attacker, weapon, levelDelta, directives });

    // attack.tnAdd from directives folds into attackerTNAdd; resist.tnAdd stays in the packet
