<script>
   ...
   const magicsDropdownOptions = $derived(ItemDataService.getAllMagics(magics));
   ...
   $effect(() => {
      if (selectedmetatype) {
         const foundItem = metatypes.find((i) => i.id === selectedmetatype);
         if (foundItem) {
            metatypeItem = foundItem;
         }
      } else {
         const fallback = metatypes.find((m) => m.name === "Human") || metatypes[0];
         if (fallback) {
   ...
   let canCreate = $derived(
      selectedmetatype && selectedMagic && selectedAttribute && selectedSkill && selectedResource
   );
   ...
   let currentPhase = $derived(agePhases.find((p) => characterAge >= p.from && characterAge <= p.to)?.text ?? "");

   let usedPriorities = $state([]);

   $effect(() => {
      const arr = [];
      const m = metatypeDropdownOptions.find((o) => o.foundryitemid === selectedmetatype);
      if (m) arr.push(m.priority);
      const g = magicsDropdownOptions.find((o) => o.foundryitemid === selectedMagic);
   ...
   async function handleSubmit(event) {
      ...
      const selectedAttributeObj = attributPointDropdownOptions.find((attr) => attr.priority === selectedAttribute);
      const selectedSkillObj = skillPointDropdownOptions.find((skill) => skill.priority === selectedSkill);

      // INFO: Six points are initiated from the data model
   ...
   function handleRandomize() {
      ...
      do {
         ...
         metaOpts = metatypeDropdownOptions.filter((i) => i.priority === combo.metatype);
         magicOpts = magicsDropdownOptions.filter((i) => i.priority === combo.magic);
      } while (!metaOpts.length || !magicOpts.length);

      selectedmetatype = metaOpts[getRandomIntinRange(0, metaOpts.length - 1)].foundryitemid;
      selectedMagic = magicOpts[getRandomIntinRange(0, magicOpts.length - 1)].foundryitemid;

      metatypeItem = metatypes.find((i) => i.id === selectedmetatype);

      const ageSrc = metatypeItem.system.agerange ?? metatypeItem.system.lifespan;
      characterAge = getRandomBellCurveWithMode(ageSrc.min, ageSrc.max, ageSrc.average);
            ...
            <div class="sr3e-inner-background-container">
               ...
               <div class="sr3e-inner-background">
                  ...
                  <input
                     id="height-slider"
                     type="range"
                     min={metatypeItem?.system?.physical?.height?.min ?? 0}
                     max={metatypeItem?.system?.physical?.height?.max ?? 200}
                     step="1"
                  ...
                  <input
                     id="weight-slider"
                     type="range"
                     min={metatypeItem?.system?.physical?.weight?.min ?? 0}
                     max={metatypeItem?.system?.physical?.weight?.max ?? 200}
                     step="1"
               ...
               <div class="sr3e-inner-background">
                  <div>
                     <div class="creation-dropdwn">
                        ...
                        <select id="metatype-select" bind:value={selectedmetatype}>
                           <option value="" disabled selected hidden>{chooseAnOption}</option>
                           ...
                           {#each magicsDropdownOptions as magic}
                              <option
                                 ...
                                 disabled={usedPriorities.includes(magic.priority) &&
                                    magic.foundryitemid !== selectedMagic}>{magic.priority}: {magic.name}</option
                              >
                           {/each}
                        </select>
                     </div>
                     <div class="creation-dropdwn">
                        ...
                        <select id="attributes-select" bind:value={selectedAttribute}>
                           <option value="" disabled selected hidden>{chooseAnOption}</option>
                           {#each attributPointDropdownOptions as attribute}
                              <option
                                 value={attribute.priority}
                                 disabled={usedPriorities.includes(attribute.priority) &&
                                    attribute.priority !== selectedAttribute}
                        ...
                        <select id="skills-select" bind:value={selectedSkill}>
                           ...
                           {#each skillPointDropdownOptions as skill}
                              <option
                                 ...
                                 disabled={usedPriorities.includes(skill.priority) && skill.priority !== selectedSkill}
