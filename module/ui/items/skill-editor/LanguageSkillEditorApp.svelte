<script lang="ts">
    ...
    const linkedAttrRating = Number(attrs["intelligence"]?.value ?? 0) + Number(attrs["intelligence"]?.modifier ?? 0);
    let committed = false;
    ...
    const specializationsStore = storeManager.GetRWStore<Array<{ name: string; value: number }>>(skill, "languageSkill.specializations");
...
<div class="skill-hud-display">
                ...
                <div class="skill-hud-title-row">
                    <h4>{localize(CONFIG.SR3E.SKILL?.readwrite)}</h4>
                    <h4>{readWrite}</h4>
                </div>
...
<div class="skill-hud-controls">
                <button
                    ...
                    onclick={decrement}
                    disabled={$isKarmaMode ? !canDecrSkill : disableControls}
                ...
                <button
                    ...
                    onclick={increment}
                    disabled={$isKarmaMode ? !canIncrSkill : disableControls}
...
{#if $specializationsStore.length > 0}
        <ItemSheetComponent>
            <div class="skill-hud-spec-header">
                <span>{localize(CONFIG.SR3E.SKILL?.lingos)}</span>
            </div>
